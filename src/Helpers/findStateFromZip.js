const findStateFromZip = (zipcode) => {
    ...
    const parsedZip = parseInt(zipcode, 10);

    // State data
    const states = [{ min: 35000, max: 36999, code: 'AL', long: "Alabama" },
    ...
    const state = states.find((s) => s.min <= parsedZip && s.max >= parsedZip);

    if (!state) {
        return false;
    }

    return { code: state.code, long: state.long };
};

export default findStateFromZip;